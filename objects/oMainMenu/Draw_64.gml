...
for (var i = 0; i < menu_count; i++) {
    ...
    if (_alpha > 0) {
        ...
        draw_sprite_stretched(sButtonMenu, option.color, btn_x + _X, current_y, btn_width, btn_height);

        // Textos del botón
        draw_set_font(global.Fonts.f1O);
        draw_set_color(c_white);
        draw_set_halign(fa_center);
        draw_text(btn_x + btn_width/2 + _X, current_y + btn_height/2 - 12, option.title);
        
        draw_set_font(global.Fonts.f1Om);
        draw_text(btn_x + btn_width/2 + _X, current_y + btn_height/2 + 18.5, option.sub);
